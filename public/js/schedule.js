  ...
  for (const eq of equipmentList) {
    ...
    for (const block of eq.blocks) {
      ...
      html += `<div class="block block-${block.blockType}" 
        ...
        title="${escapeHtml(block.customer || "")} ${block.startDate}~${block.endDate} ${BLOCK_TYPE_LABELS[block.blockType] || ""}">
        ${shortLabel}
      </div>`;
    }
    html += "</div>";
...
function openCellDetail(eqId, date, dailyStatus) {
  ...
  for (const s of dailyStatus.statuses) {
    ...
    rows += `<tr>
      ...
      <td><span class="block-badge block-${s.blockType}">${BLOCK_TYPE_LABELS[s.blockType] || s.blockType}</span></td>
